											        ...
											        <td>
											            <form action="<%=path %>/yudingTuipiao.action" name="111" method="post">
												            <input type="hidden" name="id" value="${yuding.id}">
												             <c:if test="${yuding.gaiqian eq null}">
												            <input type="submit" value="我要退票" style="width: 120px;"/>
												            </c:if>
												             <c:if test="${yuding.gaiqian ne null}">
												            <input type="submit" value="我要退票" style="width: 120px;"  disabled/>
												            </c:if>
											            </form>
											            
											             <form action="<%=path %>/yudingGaiqian.action" name="111" method="post">
												            <input type="hidden" name="id" value="${yuding.id}">
												             <input type="number"   name="gaiqian" value="${yuding.gaiqian}">
												             <c:if test="${yuding.gaiqian ne null}">
												             <input type="submit" value="改签" style="width: 120px;" disabled/>
												             </c:if>
												              <c:if test="${yuding.gaiqian eq null}">
												             <input type="submit" value="改签" style="width: 120px;"/>
												             </c:if>
												            ${yuding.gaiqianmsg}
											            </form>
											        </td>
