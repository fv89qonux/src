...
$(function () { 
	...
	$("#bookType_manage").datagrid({
		url : 'BookType/BookType_ajaxQueryBookType.action',
		fit : true,
		fitColumns : false,
		striped : true,
		rownumbers : true,
				...
				if ($("#bookTypeEditForm").form("validate")) {
					...
					if(!$("#bookTypeEditForm").form("validate")) {
						...
						$("#bookTypeEditForm").form({
						    url:"BookType/BookType_ajaxModifyBookType.action",
						    onSubmit: function(){
								if($("#bookTypeEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	bookType_manage_tool = {
		...
		exportExcel: function() {
			$("#bookTypeQueryForm").form({
			    url:"BookType/BookType_queryBookTypeOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "BookType/BookType_ajaxDeleteBookType.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "BookType/BookType_ajaxModifyBookTypeQuery.action",
					type : "post",
					data : {
